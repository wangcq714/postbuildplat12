...
class DiagRouteTable(object):
	def __init__(self):
		...
		self.pathname = None
		self.ChannalMapping = {}
		self.diagA_validDataList = []
		self.diagA_reqDataList = []
	...
	def get_valid_data(self):
		for subList in self.msgDataList:
			if subList[column_index_from_string('Q') - 1].strip(' ') == 'Y':
				self.diagA_validDataList.append(subList)
		# print(self.diagA_validDataList)
	...
	def __req_msg_element(self, dataList) -> list:
		...
		for i in range(5 - len(self.get_tx_ch_list(dataList)) + 1):
			retList.append('0u')
			retList.append('0u')
		retList.append('0u')
	...
	def data_handling(self):
		for subList in self.diagA_validDataList:
			self.diagA_reqDataList.append(self.__req_msg_element(subList))

		# print(self.diagA_reqDataList)
	...
	def buid_req_msg_list(self) -> list:
		...
		for subList in self.diagA_reqDataList:
			self.reqMsgCfgList.append('/*' + subList[column_index_from_string('A') - 1] + '*/')
			self.reqMsgCfgList.append("{")
...
class Id2IndexTable():
	...
	def __init__(self, diagRouteTable):
		"""初始化"""
		self.diagA_validDataList = diagRouteTable.diagA_validDataList
		self.idList = []
...
class WriteData(object):
	...
	def write2Proj_DiagRouter_Cfg_c(self, dataList):
		...
		if self.first_open_Proj_DiagRouter_Cfg_c == False:
			...
			with open("Proj_DiagRouter_Cfg.c",'w') as Cfile:
				for tmp in dataList:
					Cfile.write(tmp)
				Cfile.write("\n"*2)
		else:
			with open("Proj_DiagRouter_Cfg.c",'a') as Cfile:
				for tmp in dataList:
					Cfile.write(tmp)
				Cfile.write("\n"*2)
