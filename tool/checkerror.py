...
class MsgCheckError(object):
	...
	def literal_check(self):
		...
		for i in range(len(dataList)):
			...
			# 防止有效数据行数小于配置行数
			if dataList[i][headerList.index("源报文ID")] != "None":
	...
	def logic_check(self):
		...
		for ind, subList in enumerate(dataList):
			row = ind +3
			# 防止有效数据行数小于配置行数
			if subList[headerList.index("源报文ID")] != "None":
				# 进行单行报文数据校验
...
class SignalCheckError(object):
	...
	def literal_check(self):
		...
		for i in range(len(dataList)):
			...
			# 防止有效数据行数小于配置行数
			if dataList[i][headerList.index("No")] != "None":
	...
	def logic_check(self):
		...
		for ind, subList in enumerate(dataList):
			row = ind +3
			# 防止有效数据行数小于配置行数
			if subList[headerList.index("No")] != "None":
				# 进行单行报文数据校验
